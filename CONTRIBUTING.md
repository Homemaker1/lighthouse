...
```js
const createExpectedError = require('./lighthouse-core/lib/expected-error');

if (options.url.includes('uncool')) {
  throw createExpectedError(new Error('Your site is not cool!'));
}
```
